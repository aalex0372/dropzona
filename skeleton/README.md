...
delete the folder and the only ripples are in `index.html`, `login.html`,
`app.html`, and `eslint.config.js` / `package.json` (lint glob).
...
| `LoginSkeleton.js`      | `mountLoginSkeleton(step?)`, `unmountLoginSkeleton()`         | `step` reads from `?step=role\|viewer\|streamer` if omitted. Host: `#login-skeleton`. |
...
Both `mount*` functions are idempotent (won't double-mount).
...
| `LoginSkeleton.js`  | ~140 | Auth flow, 3 step variants                                   |
| `AppSkeleton.js`    | ~430 | SPA dispatcher + 12 page builders + shared atoms             |
| `skeleton.css`      | ~700 | Shimmer + per-page layout                                    |
...
- [x] Three HTML files use it (`index.html`, `login.html`, `app.html`)
- [x] `npm run lint` passes
- [x] `?skeleton=hold` / `?skeleton=<ms>` debug params work on all three pages
